...
RUN mvn install:install-file -Dfile=dependencies/securityfilter.jar -DgroupId=securityfilter -DartifactId=securityfilter -Dversion=1.0 -Dpackaging=jar && \
    ...
    mvn install:install-file -Dfile=dependencies/ispyb-userportal-gen-1.10.jar -DgroupId=ispyb -DartifactId=ispyb-userportal-gen -Dversion=1.10 -Dpackaging=jar && \

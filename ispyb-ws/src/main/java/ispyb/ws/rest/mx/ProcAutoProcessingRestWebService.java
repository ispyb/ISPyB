...
import ispyb.server.mx.services.utils.bean.AutoProcAttachmentWebBean;
import ispyb.server.mx.services.utils.bean.AutoProcessingDetail;
...
public class ProcAutoProcessingRestWebService extends MXRestWebService {
	...
	public Response getAnalysisByAutoProcIntegrationId(@PathParam("token") String token,
		...
		try {
			...
			List<Map<String, Object>> result = external3Service
					.getPhasingAnalysisByDataCollectionIdListQuery(autoProcIntegrationId);
	...
	@RolesAllowed({ "User", "Manager", "LocalContact" })
	@GET
	@Path("{token}/proposal/{proposal}/mx/autoprocintegration/{autoProcIntegrationListId}/xscale/attachment/list")
	@Produces("text/plain")
...
//						recordautoIntegrationsProgram.put("datacollection", this.getDataCollection3Service().findByPk(autoProcIntegration3VO.getDataCollectionVOId(), false, false, false));
	...
	@RolesAllowed({ "User", "Manager", "LocalContact" })
	@GET
	@Path("{token}/proposal/{proposal}/mx/autoproc/autoprocattachmentid/{autoProcAttachmentId}/download")
	@Produces({ "application/json" })

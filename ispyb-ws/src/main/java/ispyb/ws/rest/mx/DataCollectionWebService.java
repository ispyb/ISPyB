...
public class DataCollectionWebService extends MXRestWebService {
	...
	public Response getDataCollectionById(@PathParam("token") String token,
		...
		try {
			...
			for (Integer id : ids) {
				dataCollections.addAll(this.getWebServiceDataCollection3Service().getViewDataCollectionByDataCollectionId(this.getProposalId(proposal), id));
			}
	...
	public Response getViewDataCollectionBySessionId(
		...
		try {
			...
			for (Integer id : ids) {
				dataCollections.addAll(this.getWebServiceDataCollection3Service().getViewDataCollectionBySessionId(this.getProposalId(proposal), id));
			}
			this.logFinish(methodName, start, logger);
			return this.sendResponse(dataCollections, false);
		} catch (Exception e) {

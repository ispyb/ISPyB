...
public class ProteinRestWebService extends MXRestWebService {
	...
	@RolesAllowed({"User", "Manager", "Industrial", "Localcontact"})
	@POST
	@Path("/{token}/proposal/{proposal}/mx/protein/savewithcrystalform")
	@Produces({ "application/json" })
	public Response saveProteinWithEmptyCrystalForm(
			@PathParam("token") String token,
			@PathParam("proposal") String proposal,
			@FormParam("proteinId") Integer proteinId,
			@FormParam("name") String name,
		...
		try {
			...
			if (createEmptyCrystalForm == 1){
				...
				crystal.setCellA(new Double(0));
				crystal.setCellB(new Double(0));
				crystal.setCellC(new Double(0));
				crystal.setCellAlpha(new Double(0));
				crystal.setCellBeta(new Double(0));
				crystal.setCellGamma(new Double(0));
				crystal.setProteinVO(protein3vo);
				logger.info("Creating dummy crystal form");
				crystal = this.getCrystal3Service().create(crystal);
			}
			this.logFinish("saveProteinWithEmptyCrystalForm", start, logger);

			return sendResponse(protein3vo);
		} catch (Exception e) {
			this.logError("saveProteinWithEmptyCrystalForm", e, start, logger);
		}
		return null;
	}

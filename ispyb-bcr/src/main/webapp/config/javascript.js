...
function storeValue(caller) {
setCookie(caller.name, caller.value);
}
...
function arrayCompare(e1,e2) {
	// Ludovic : Sort numbers correctly + Lower Case comparison
	if (isNumber(e1) && isNumber(e2))
		{
		return parseInt(e1[0]) < parseInt(e2[0]) ? -1 : (parseInt(e1[0]) == parseInt(e2[0]) ? 0 : 1);
		}
	return e1[0].toLowerCase() < e2[0].toLowerCase() ? -1 : (e1[0].toLowerCase() == e2[0].toLowerCase() ? 0 : 1);
}
...
function showLayoutLayer(ID,event) {
	...
	var layerPosY = event.clientY;
	var layerPosX = event.clientX;	

 ...
 */
@Stateless
public class Position3ServiceBean implements Position3Service, Position3ServiceLocal {

	private final static Logger LOG = LogManager.getLogger(Position3ServiceBean.class);

	// Generic HQL request to find instances of Position3 by pk
	// TODO choose between left/inner join
	private static final String FIND_BY_PK() {
		return "from Position3VO vo " + "where vo.positionId = :pk";
	}
	...
	public Position3VO findByPk(final Integer pk) throws Exception {
		...
		// TODO Edit this business code
		try {
			return (Position3VO) entityManager.createQuery(FIND_BY_PK()).setParameter("pk", pk).getSingleResult();
	...
	public List<Position3VO> findAll() throws Exception {
		...
		List<Position3VO> foundEntities = (List<Position3VO>) entityManager.createQuery(FIND_ALL()).getResultList();
		return foundEntities;
	}

	/**
	 * Check if user has access rights to create, change and remove Position3
	 ...
	 */
	private void checkAndCompleteData(Position3VO vo, boolean create) throws Exception {

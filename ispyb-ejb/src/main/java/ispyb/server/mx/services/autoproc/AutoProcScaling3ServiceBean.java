 ...
 */
@Stateless
public class AutoProcScaling3ServiceBean implements AutoProcScaling3Service, AutoProcScaling3ServiceLocal {

	private final static Logger LOG = LogManager.getLogger(AutoProcScaling3ServiceBean.class);
	...
	public AutoProcScaling3VO findByPk(final Integer pk) throws Exception {
		...
		checkCreateChangeRemoveAccess();
		try {
			return (AutoProcScaling3VO) entityManager.createQuery(FIND_BY_PK()).setParameter("pk", pk)
					.getSingleResult();
	...
	public List<AutoProcScaling3VO> findAll() throws Exception {
	...
	/**
	 * Check if user has access rights to create, change and remove AutoProcScaling3

 ...
 */
@Stateless
public class AutoProcIntegration3ServiceBean implements AutoProcIntegration3Service, AutoProcIntegration3ServiceLocal {

	private final static Logger LOG = LogManager.getLogger(AutoProcIntegration3ServiceBean.class);
	...
	private static final String FIND_AUTOPROC_STATUS = "SELECT app.processingPrograms, api.autoProcIntegrationId, apppa.fileName "
			...
			+ "apppa.autoProcProgramId = app.autoProcProgramId AND " + "apppa.fileName like '%XSCALE%' ";

	/*
	 * XIA2_DIALS don't create any XSCALE files so here we look for a file with the
	 * prefix 'di' and the suffix '.mtz'.
	 */
	private static final String FIND_XIA2_DIALS_STATUS = "SELECT app.processingPrograms, api.autoProcIntegrationId, apppa.fileName "
			...
			+ "api.autoProcProgramId = app.autoProcProgramId AND "
			+ "app.processingPrograms like :processingProgram AND  "
			+ "apppa.autoProcProgramId = app.autoProcProgramId AND " + "apppa.fileName like 'di%.mtz' ";

	/*
	 * FAST DP don't create any XSCALE files so here we look for a file with the
	 * prefix 'fast' and the suffix '.mtz.gz'.
	 */
	private static final String FIND_FASTDP_STATUS = "SELECT app.processingPrograms, api.autoProcIntegrationId, apppa.fileName "
			...
			+ "api.autoProcProgramId = app.autoProcProgramId AND "
			+ "app.processingPrograms like :processingProgram AND  "
			+ "apppa.autoProcProgramId = app.autoProcProgramId AND " + "apppa.fileName like '%fast_dp%.mtz.gz' ";
	...
	public AutoProcIntegration3VO findByPk(final Integer pk) throws Exception {
		...
		checkCreateChangeRemoveAccess();
		try {
			return (AutoProcIntegration3VO) entityManager.createQuery(FIND_BY_PK()).setParameter("pk", pk)
					.getSingleResult();
	 ...
	 * AutoProcIntegration3 entities. If not set rollback only and throw
	...
	public List<AutoProcIntegration3VO> findByAutoProcId(final Integer autoProcId) throws Exception {
		...
		List<AutoProcIntegration3VO> listVOs = this.entityManager
				.createNativeQuery(query, "autoProcIntegrationNativeQuery").setParameter("autoProcId", autoProcId)
				.getResultList();
		return listVOs;
	}

	/**
	 * get the autoProcStatus for a given dataCollectionId and a given
	 * processingProgram (fastProc or parallelProc or edna-fastproc) true if we can
	 * find at least one autoProc with XSCALE file
	 * 
	...
	public Boolean getAutoProcStatus(final Integer dataCollectionId, final String processingProgram) throws Exception {
		...
		try {
			List res = this.entityManager.createNativeQuery(query).setParameter("dataCollectionId", dataCollectionId)
					.setParameter("processingProgram", processingProgram).getResultList();
	 ...
	 * @throws Exception
	 */
	public Boolean getAutoProcXia2DialsStatus(final Integer dataCollectionId, final String processingProgram)
			throws Exception {
	 ...
	 * @throws Exception
	 */
	public Boolean getAutoProcFastDPStatus(final Integer dataCollectionId, final String processingProgram)
			throws Exception {

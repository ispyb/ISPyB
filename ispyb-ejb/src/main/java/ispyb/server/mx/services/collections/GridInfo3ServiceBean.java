 ...
 */
@Stateless
public class GridInfo3ServiceBean implements GridInfo3Service, GridInfo3ServiceLocal {

	private final static Logger LOG = LogManager.getLogger(GridInfo3ServiceBean.class);

	// Generic HQL request to find instances of GridInfo3 by pk
	// TODO choose between left/inner join
	private static final String FIND_BY_PK() {
		return "from GridInfo3VO vo " + "where vo.gridInfoId = :pk";
	}
	...
	public GridInfo3VO findByPk(final Integer pk) throws Exception {
		...
		// TODO Edit this business code
		try {
			return (GridInfo3VO) entityManager.createQuery(FIND_BY_PK()).setParameter("pk", pk).getSingleResult();
	...
	public List<GridInfo3VO> findAll() throws Exception {
		...
		List<GridInfo3VO> foundEntities = (List<GridInfo3VO>) entityManager.createQuery(FIND_ALL()).getResultList();
		return foundEntities;
	}

	/**
	 * Check if user has access rights to create, change and remove GridInfo3
	 ...
	 */
	private void checkAndCompleteData(GridInfo3VO vo, boolean create) throws Exception {

 ...
 */
@Stateless
public class IspybAutoProcAttachment3ServiceBean
		implements IspybAutoProcAttachment3Service, IspybAutoProcAttachment3ServiceLocal {

	private final static Logger LOG = LogManager.getLogger(IspybAutoProcAttachment3ServiceBean.class);
	...
	public IspybAutoProcAttachment3VO findByPk(final Integer pk) throws Exception {
		...
		checkCreateChangeRemoveAccess();
		try {
			return (IspybAutoProcAttachment3VO) entityManager.createQuery(FIND_BY_PK()).setParameter("pk", pk)
					.getSingleResult();
	...
	public List<IspybAutoProcAttachment3VO> findAll() throws Exception {
	 ...
	 * IspybAutoProcAttachment3 entities. If not set rollback only and throw

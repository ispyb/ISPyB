...
public class SampleAverageReaderFactory extends AbstractReaderFactory {
	...
	List<HashMap<String, Object>> getData(List<Integer> idList)
		...
		for (Integer subtractionId : idList) {
			...
			if (subtraction != null){
				result.add(new OneDimensionalFileReader(subtraction.getSubtractionId(), subtraction.getSampleAverageFilePath()).readFile());
			}
		}
		return result;
	}

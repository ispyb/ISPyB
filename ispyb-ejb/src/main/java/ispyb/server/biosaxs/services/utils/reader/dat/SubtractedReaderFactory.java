...
public class SubtractedReaderFactory extends AbstractReaderFactory {
	...
	public List<HashMap<String, Object>> getData(List<Integer> idList) throws Exception {
		...
		for (Integer subtractionId : idList) {
			...
			if (subtraction != null){
				result.add(new OneDimensionalFileReader(subtraction.getSubtractionId(), subtraction.getSubstractedFilePath()).readFile());
			}
		}
		return result;
	}

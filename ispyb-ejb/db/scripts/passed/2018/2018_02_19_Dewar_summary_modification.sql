...
VIEW `v_dewar_summary` AS
    SELECT 
        ...
        `Dewar`.`type` AS `type`,
        `Dewar`.`isReimbursed` AS `isReimbursed`,
        ...
        `BLSession`.`nbReimbDewars` AS `nbReimbDewars`,
...
update SchemaStatus set schemaStatus = 'DONE' where scriptName = '2018_02_18_Dewar_summary_modification.sql';